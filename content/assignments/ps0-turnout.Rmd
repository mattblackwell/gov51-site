...
Create two new variables in the data set:
  ...
  - `turnout$VEtr`: the turnout rate (as a percentage) based on the
  voting eligible population or VEP.

 Create a table using `knitr::kable()` with columns for year, both
  types of turnout rates as columns. What difference do you observe?
...
Create two vectors of the ANES-VEP difference in turnout rates that contain presidential election years (1980, 1984, ...) and midterm election years  (1982, 1986, ...) separately. Call these vectors `pres_diffVEP` and `mid_diffVEP`. Note that the data set excludes the year 2006. How does the average bias of the ANES vary across election types, if at all?
